...
public class IHDRBlock extends DataBlock {
    ...
    public void setData(byte[] data) {
        int pos = 0;
        this.width = ByteUtils.cutByte(data, pos, this.width.length);
        pos += this.width.length;
        this.height = ByteUtils.cutByte(data, pos, this.height.length);
        pos += this.height.length;
        this.bitDepth = ByteUtils.cutByte(data, pos, this.bitDepth.length);
        pos += this.bitDepth.length;
        this.colourType = ByteUtils.cutByte(data, pos, this.colourType.length);
        pos += this.colourType.length;
        this.compressionMethod = ByteUtils.cutByte(data, pos, this.compressionMethod.length);
        pos += this.compressionMethod.length;
        this.filterMethod = ByteUtils.cutByte(data, pos, this.filterMethod.length);
        pos += this.filterMethod.length;
        this.interlaceMethod = ByteUtils.cutByte(data, pos, this.interlaceMethod.length);
